...
| Language and presentation | `src/shared/language/`, `src/shared/theme/`, `src/shared/playback-highlight/`                   | Own the active language, direction, theme, and playback highlighting. UI text comes from Paraglide messages.                                                                  |
...
| Persisted settings           | `createPersistedStore` + localStorage          | Language, speech, highlighting, board appearance, AI context, and onboarding state. MUI theme mode persists separately as `mui-mode` and is read pre-paint in `index.html`. |
