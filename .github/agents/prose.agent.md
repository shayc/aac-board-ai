...
- **Restore the Smothered Verb (Nominalization):** Beware the action frozen into a noun — the `Manager`, the `Processor`, the `-ationService` whose only real content is one buried method. A type that is merely a strangled verb should become that verb: `EmailNotificationManager.perform()` is just `notify()`. Ask the plain question — _who does what?_ — then put the action in a verb and the actor in the object, never in a ceremonial noun that hides both.
...
- **Chekhov's Gun (No Idle Setup):** Every element you introduce promises the reader it matters. A parameter never read, a variable never used, an import that resolves to nothing, a flag nothing branches on, an abstraction with a single caller — each is a rifle hung on the wall that never fires. Take it down or fire it; never make the reader hold a thread that will not pay off.
